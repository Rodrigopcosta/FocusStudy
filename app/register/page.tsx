...
export default function RegisterPage() {
  ...
  const isNameInvalid =
    nameTouched && name.trim().length > 0 && name.trim().length < 2
  const isEmailInvalid =
    emailTouched &&
    email.length > 0 &&
    !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)
  const isBirthDateInvalid =
    birthDateTouched && (birthDate === '' || !validateAge(birthDate))
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (
      ...
      setError(
        'A senha não atende aos requisitos (mín. 6 caracteres, maiúscula e caractere especial).'
      )
      setIsLoading(false)
      return
    }
    ...
    try {
      ...
      if (error.message?.includes('rate limit')) {
        ...
        setError(
          'Erro interno ao salvar perfil. Verifique as configurações do banco de dados.'
        )
      } else {
        setError(
  ...
  const RuleItem = ({ met, text }: { met: boolean; text: string }) => (
    <div
      className={`flex items-center gap-1.5 text-[11px] font-medium transition-colors ${met ? 'text-green-500' : 'text-muted-foreground'}`}
    >
      {met ? (
      ...
      <div className="w-full max-w-md">
        <div className="mb-8 flex items-center justify-between">
          <Button
            variant="outline"
            asChild
            className="group px-4 border-muted-foreground/20 text-foreground cursor-pointer"
          >
            <Link href="/">
            ...
            <CardContent>
              ...
              <form onSubmit={handleSignUp} className="space-y-4">
                <div className="grid gap-2">
                  <Label
                    htmlFor="name"
                    className={`cursor-pointer ${isNameInvalid ? 'text-destructive' : ''}`}
                  >
                    Nome completo *
                  </Label>
                  <Input
                ...
                <div className="grid gap-2 relative">
                  <Label
                    htmlFor="email"
                    className={`cursor-pointer ${isEmailInvalid ? 'text-destructive' : ''}`}
                  >
                    E-mail *
                  </Label>
                  <Input
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    <Label
                      htmlFor="birthDate"
                      className={`cursor-pointer ${isBirthDateInvalid ? 'text-destructive' : ''}`}
                    >
                      Nascimento *
                    </Label>
                    <Input
                  ...
                  <div className="grid gap-2">
                    ...
                    >
                      <SelectTrigger
                        className={`h-11 cursor-pointer ${isGenderInvalid ? 'border-destructive focus-visible:ring-destructive' : ''}`}
                      >
                ...
                <div className="grid gap-2">
                  <Label htmlFor="studyType" className="cursor-pointer">
                    Área de Concurso *
                  </Label>
                  <Select value={studyType} onValueChange={setStudyType}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="policial" className="cursor-pointer">
                        Policial
                      </SelectItem>
                      <SelectItem value="tribunais" className="cursor-pointer">
                        Tribunais / Judiciária
                      </SelectItem>
                      <SelectItem value="fiscal" className="cursor-pointer">
                        Fiscal / Controle
                      </SelectItem>
                      <SelectItem value="saude" className="cursor-pointer">
                        Saúde
                      </SelectItem>
                      <SelectItem value="educacao" className="cursor-pointer">
                        Educação
                      </SelectItem>
                      <SelectItem value="outros" className="cursor-pointer">
                        Outros
                      </SelectItem>
                    </SelectContent>
                  </Select>
                ...
                <div className="grid gap-4">
                  <div className="grid gap-2">
                    <Label
                      htmlFor="password"
                      title="Clique para focar na senha"
                      className="cursor-pointer"
                    >
                      Senha *
                    </Label>
                ...
                {error && (
                  <p className="text-sm font-medium text-destructive text-center bg-destructive/10 p-2 rounded-md animate-in fade-in zoom-in">
                    {error}
                  </p>
                )}
                ...
                <Button
                  type="submit"
                  className={`w-full h-11 text-base font-semibold transition-all active:scale-[0.98] ${isLoading || isGoogleLoading ? 'cursor-not-allowed' : 'cursor-pointer'}`}
