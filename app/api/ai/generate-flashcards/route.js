...
export async function POST(req) {
  try {
        ...
        return NextResponse.json(
          {
            error: 'limit_reached',
            message:
              'Usuários gratuitos podem gerar apenas 5 flashcards por dia.',
          },
          { status: 403 }
        )
      }
      ...
      return NextResponse.json(
        {
          error: 'invalid_content',
          message:
            'Não consegui identificar um assunto educativo no seu texto. Tente enviar um parágrafo mais claro.',
        },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      { error: 'Falha ao gerar flashcards', details: error.message },
      { status: 500 }
    )
  }
}
