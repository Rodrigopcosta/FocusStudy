      ...
      <header className="border-b border-border/40 bg-background/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="container mx-auto px-4 py-3 md:py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 group cursor-pointer">
            ...
            <span className="font-black text-xl md:text-2xl tracking-tighter italic uppercase">FocusStudy</span>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <Button asChild className="shadow-xl shadow-primary/20 font-black uppercase italic text-[10px] md:text-xs rounded-full px-5 md:px-8 h-10 md:h-11 border-2 border-primary/20">
      ...
      <main>
            ...
            <div className="flex flex-col items-center space-y-8 md:space-y-10">
              ...
              <div className="space-y-4 md:space-y-6">
                ...
                <p className="text-muted-foreground text-base md:text-2xl font-medium max-w-2xl mx-auto leading-relaxed italic px-4">
                  ...
                  O método de elite feito para o <span className="text-foreground font-bold underline decoration-primary underline-offset-4">máximo desempenho.</span>
                </p>
              </div>

              <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md sm:max-w-none justify-center pt-4 px-6 sm:px-0">
                <Button size="lg" asChild className="h-16 md:h-20 px-8 md:px-12 rounded-full font-black uppercase italic text-base md:text-lg bg-primary hover:scale-105 transition-all shadow-[0_20px_60px_rgba(var(--primary-rgb),0.35)]">
                ...
                <Button size="lg" variant="outline" asChild className="h-16 md:h-20 px-8 md:px-12 rounded-full font-black uppercase italic text-base md:text-lg border-2 hover:bg-secondary/50 backdrop-blur-sm">
              ...
              <div className="pt-12 md:pt-16 flex flex-wrap justify-center gap-x-8 gap-y-6 md:gap-x-12 opacity-40 grayscale hover:grayscale-0 transition-all duration-500 px-4">
                <div className="flex items-center gap-2 font-black italic uppercase text-[9px] md:text-[10px] tracking-widest"><BrainCircuit className="h-4 w-4 md:h-5 md:w-5" /> Inteligência</div>
                <div className="flex items-center gap-2 font-black italic uppercase text-[9px] md:text-[10px] tracking-widest"><Focus className="h-4 w-4 md:h-5 md:w-5" /> Foco</div>
                <div className="flex items-center gap-2 font-black italic uppercase text-[9px] md:text-[10px] tracking-widest"><TrendingUp className="h-4 w-4 md:h-5 md:w-5" /> Evolução</div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-20 md:py-32 container mx-auto px-4">
          <div className="bg-primary p-10 md:p-24 rounded-[2.5rem] md:rounded-[3.5rem] text-primary-foreground text-center space-y-6 md:space-y-8 relative overflow-hidden shadow-3xl shadow-primary/20">
            ...
            <Button size="lg" variant="secondary" asChild className="h-14 md:h-16 px-8 md:px-12 rounded-full font-black uppercase italic text-base md:text-lg hover:scale-105 transition-transform relative z-10 shadow-2xl w-full sm:w-auto">
      ...
      <footer className="border-t border-border/50 bg-card/50 py-12 md:py-20">
        <div className="container mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-8 md:gap-10 text-center md:text-left">
          ...
          <div className="flex gap-6 md:gap-10 text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-muted-foreground/60">
            <Link href="/terms" className="hover:text-primary transition-colors italic cursor-pointer">Termos</Link>
            <Link href="/privacy" className="hover:text-primary transition-colors italic cursor-pointer">Privacidade</Link>
            <Link href="#" className="hover:text-primary transition-colors italic cursor-pointer">Suporte</Link>
          </div>
...
function FeatureCard({ icon: Icon, title, description }: { icon: any, title: string, description: string }) {
  return (
    <div className="group p-8 md:p-10 rounded-4xl md:rounded-[2.5rem] bg-card border border-border/50 hover:border-primary/40 transition-all hover:shadow-2xl hover:shadow-primary/5">
      ...
      <h3 className="text-lg md:text-xl font-black uppercase italic mb-3 md:mb-4 tracking-tight">{title}</h3>
      <p className="text-muted-foreground text-xs md:text-sm font-medium leading-relaxed italic">{description}</p>
    </div>
  )
}