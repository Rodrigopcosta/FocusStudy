...
export default function FocusModePage() {
  ...
  const [mode, setMode] = useState<PomodoroMode>((searchParams.get("mode") as PomodoroMode) || "25/5")
    ...
    try {
      ...
      if (minutesStudied > 0) {
        await supabase.rpc('increment_study_stats', { 
          user_id: user.id, inc_xp: xpGained, inc_minutes: minutesStudied 
        })
      }
    } finally {
  ...
  return (
    <div className={`fixed inset-0 z-9999 flex flex-col items-center justify-center transition-colors duration-500 ${isBreak ? "bg-emerald-500/10" : "bg-background"}`}>
      {/* Botão X da Esquerda (Print 2) */}
      <Button variant="ghost" size="icon" className="absolute top-4 left-4" onClick={handleExit}>
        <X className="h-6 w-6" />
      </Button>

      <div className="text-center">
        <div className="flex items-center justify-center gap-2 mb-8">
          {isBreak ? <Coffee className="h-6 w-6 text-emerald-500" /> : <Target className="h-6 w-6 text-primary" />}
          <span className="text-lg font-medium">{isBreak ? "Descanso" : "Foco"}</span>
        </div>

        <div className="relative mb-12 flex items-center justify-center">
          <svg className="w-80 h-80 transform -rotate-90">
            <circle cx="160" cy="160" r="140" stroke="currentColor" strokeWidth="8" fill="none" className="text-muted" />
            <circle
              cx="160" cy="160" r="140" stroke="currentColor" strokeWidth="8" fill="none"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-7xl font-bold font-mono tabular-nums">{formatTime(timeLeft)}</span>
            <span className="text-muted-foreground mt-2">{completedCycles} ciclos concluídos</span>
          </div>
        </div>

        <div className="flex items-center justify-center gap-4">
          <Button size="lg" onClick={isRunning ? () => setIsRunning(false) : handleStart} className="h-14 px-8">
            {isRunning ? <Pause className="mr-2 h-5 w-5" /> : <Play className="mr-2 h-5 w-5" />}
            {isRunning ? "Pausar" : "Iniciar"}
          </Button>
          <Button size="lg" variant="outline" onClick={() => setTimeLeft(MODES[mode].work)} className="h-14 px-8">
