...
const BADGES_MASTER = [
  ...
  },
  {
    id: 'semana-ouro',
    name: 'Semana de Ouro',
    description: '7 dias de ofensiva seguidos',
    icon: '🔥',
    category: 'consistency',
    target: 7,
  },
  {
    id: 'inabalavel',
    name: 'Inabalável',
    description: '30 dias de ofensiva sem falhar',
    icon: '💎',
    category: 'consistency',
    target: 30,
  },
  {
    ...
    description: '1.000 horas líquidas de estudo',
    icon: '🏆',
    category: 'consistency',
    target: 1000,
  },
  {
    id: 'nota-inicial',
    name: 'Nota Inicial',
    description: 'Escreveu seu primeiro resumo',
    icon: '📝',
    category: 'discipline',
    target: 1,
  },
  {
    id: 'estudioso',
    name: 'Estudioso',
    description: 'Concluiu 20 resumos',
    icon: '📚',
    category: 'discipline',
    target: 20,
  },
  {
    ...
    description: '100 horas líquidas de estudo',
    icon: '⌛',
    category: 'discipline',
    target: 100,
  },
  {
    id: 'doutrinador',
    name: 'Doutrinador',
    description: 'Criou 500 resumos detalhados',
    icon: '📜',
    category: 'discipline',
    target: 500,
  },
  ...
  },
  {
    id: 'flash-hunter',
    name: 'Flash Hunter',
    description: 'Respondeu 50 Flashcards',
    icon: '⚡',
    category: 'speed',
    target: 50,
  },
  {
    id: 'sniper-questoes',
    name: 'Sniper de Questões',
    description: '90% de acerto em Flashcards',
    icon: '🏹',
    category: 'speed',
    target: 90,
  },
  {
    id: 'cerebro-silicio',
    name: 'Cérebro de Silício',
    description: '10.000 Flashcards revisados',
    icon: '🧠',
    category: 'speed',
    target: 10000,
  },
...
export default function JornadaPage() {
  ...
  const getMultiplier = (streak: number) => {
    if (streak >= 15)
      return { label: '2.0x', color: 'text-purple-500', bg: 'bg-purple-500/10' }
    if (streak >= 7)
      return { label: '1.5x', color: 'text-orange-500', bg: 'bg-orange-500/10' }
    if (streak >= 3)
      return { label: '1.2x', color: 'text-blue-500', bg: 'bg-blue-500/10' }
    return {
      label: '1.0x',
  ...
  useEffect(() => {
    async function loadJornadaData() {
      try {
        ...
        const [profileRes, badgesRes, tasksRes, rankingRes, focusRes] =
          await Promise.all([
            supabase
              .from('profiles')
              .select('xp, level, streak_current, plan_type')
              .eq('id', user.id)
              .maybeSingle(),
            supabase
              .from('user_badges')
        ...
        const badgeIds =
          (badgesRes.data as UserBadge[])?.map((b: UserBadge) => b.badge_id) ||
          []
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
        {filtered.map(badge => (
          <BadgeCard
            ...
            badge={{
              ...badge,
              unlockedAt: unlockedBadges.includes(badge.id)
                ? 'Conquistado'
                : undefined,
        ...
        <div className="space-y-1 text-center lg:text-left">
          ...
          <div className="flex items-center justify-center lg:justify-start gap-2">
            <p className="text-sm text-muted-foreground font-medium italic">
              "A constância vence a inteligência."
            </p>
            <div
              className={`flex items-center gap-1 px-2 py-0.5 rounded-full border border-current/20 text-[10px] font-bold uppercase tracking-tighter ${bonus.bg} ${bonus.color}`}
            >
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="bg-card p-6 rounded-3xl border border-border/60 shadow-sm space-y-6">
                ...
                <Tabs defaultValue="all">
                  ...
                  <TabsContent value="consistency">
                    {renderBadgeGrid('consistency')}
                  </TabsContent>
                  <TabsContent value="discipline">
                    {renderBadgeGrid('discipline')}
                  </TabsContent>
                  <TabsContent value="speed">
                    {renderBadgeGrid('speed')}
                  </TabsContent>
                </Tabs>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              >
                <h3
                  className={`font-black uppercase flex items-center gap-2 mb-2 ${userData.streak > 0 ? 'text-orange-600' : 'text-muted-foreground'}`}
                >
                  <Flame
                    className={`h-5 w-5 ${userData.streak > 0 ? 'animate-pulse' : ''}`}
                  />
                  Ofensiva {userData.streak} Dias
                </h3>
                <p
                  className={`text-sm ${userData.streak > 0 ? 'text-orange-600/80' : 'text-muted-foreground/60'}`}
                >
                  {userData.streak > 0
            ...
            {!isPremium ? (
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-6 bg-linear-to-b from-transparent to-primary/5">
                ...
                <div className="max-w-sm space-y-2">
                  ...
                  <p className="text-muted-foreground font-medium text-sm">
                    Apenas membros **PRO** podem ver o Ranking Global e disputar
                    o topo do Hall da Fama.
                  </p>
                </div>
                <Button
              ...
              <div className="divide-y divide-border/40">
                {topUsers.map((user, index) => (
                  <div
                    key={user.id}
                    className={`flex items-center justify-between p-5 transition-colors ${user.id === userData.id ? 'bg-primary/10' : 'hover:bg-secondary/5'}`}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p
                          className={`font-bold ${user.id === userData.id ? 'text-primary' : ''}`}
                        >
                          {user.full_name || 'Estudante'}{' '}
                          {user.id === userData.id && '(Você)'}
                        </p>
                        <p className="text-[10px] uppercase font-bold text-orange-500 flex items-center gap-1">
                          <Flame className="h-3 w-3" /> {user.streak_current}d
                          de ofensiva
                        </p>
                      </div>
                    </div>
