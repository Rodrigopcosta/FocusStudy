...
export default function FlashcardsPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorMessage =
          data?.message || data?.error || 'Erro desconhecido na geração'

        if (data?.error === 'invalid_content' || response.status === 400) {
          toast.error('Conteúdo Inválido', {
            description:
              data?.message ||
              'O texto não possui informações claras para criar estudos.',
          })
    ...
    <div className="flex-1 space-y-6 p-4 md:p-8 pt-6 max-w-400 mx-auto">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-muted-foreground text-sm md:text-base font-medium">
          Gere e gerencie seu material de estudo.
        </p>
      </div>
          ...
          <Card className="border-2 shadow-sm">
            <CardHeader className="p-4 md:p-6">
              ...
              <CardDescription className="text-xs md:text-sm font-medium italic">
                Cole o texto para a IA processar.
              </CardDescription>
            </CardHeader>
              ...
              >
                {isGenerating ? (
                  ...
                  <>
                    ...
                    <span className="hidden sm:inline italic font-black">
              ...
              <div className="flex items-center justify-between">
                ...
                {flashcards.length > 0 && (
                  <Button
                    ...
                    className="h-8 px-2 text-xs md:text-sm font-bold text-destructive hover:text-destructive/80 cursor-pointer"
                  >
              ...
              {flashcards.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center py-24 text-muted-foreground/50 italic">
                  ...
                  <p className="text-sm font-black uppercase tracking-widest text-center px-4">
                    Aguardando geração inteligente...
                  </p>
                </div>
              ) : (
                <>
                  <div className="divide-y divide-border">
                    {flashcards.map((card, index) => (
                      <div
                        key={index}
                        className="p-5 md:p-8 hover:bg-accent/30 transition-colors border-b last:border-0"
                      >
                        <div className="space-y-4">
                          {/* Campo da Frente com Máscara Visual */}
                          <div className="flex flex-col gap-2">
                            <span className="shrink-0 text-[10px] font-black bg-blue-500 text-white px-2 py-0.5 rounded w-fit shadow-sm uppercase">
                              Frente
                            </span>
                            <textarea
                          ...
                          {/* Campo do Verso */}
                          <div className="flex flex-col gap-2">
                            <span className="shrink-0 text-[10px] font-black bg-purple-500 text-white px-2 py-0.5 rounded w-fit shadow-sm uppercase">
                              Verso
                            </span>
                            <input
