            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-1.5 mt-2">
                {task.discipline && (
                  <Badge
                    variant="secondary"
                    style={{
                      backgroundColor: `${task.discipline.color}15`,
                      color: task.discipline.color,
                    }}
                    className="text-[9px] sm:text-[10px] font-bold border-none px-1.5 py-0"
                  >
                    <span className="mr-1">{task.discipline.icon}</span>
                    {task.discipline.name}
                  </Badge>
                )}
                <Badge
                  variant="outline"
                  className="text-[9px] sm:text-[10px] uppercase tracking-wider px-1.5 py-0"
                >
                  {typeLabels[task.type as keyof typeof typeLabels]}
                </Badge>
                <Badge
                  className={cn(
                    'text-[9px] sm:text-[10px] border-transparent px-1.5 py-0',
              ...
              <div className="flex items-center justify-between mt-3 pt-2 border-t border-dashed border-muted">
                <div className="flex items-center gap-2 text-muted-foreground text-[10px] sm:text-[11px]">
                  ...
                  <div className="flex items-center gap-1 font-bold text-foreground bg-secondary/50 px-2 py-0.5 rounded-full">
                ...
                {task.status === 'pending' && !isReorderMode && (
                  <Button
                    ...
                    size="icon"
                    asChild
                    className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-primary text-white shadow-sm active:scale-95 transition-all"
                  >
      ...
      {isReorderMode && showMoveButton && (
        <div className="flex justify-center -mb-5 mt-1 z-50 relative">
          <Button
            variant="secondary"
            size="icon"
            className="rounded-full h-8 w-8 sm:h-10 sm:w-10 shadow-lg border-2 border-primary bg-background"
...
export function TaskList({ tasks: initialTasks, disciplines }: TaskListProps) {
  ...
  const processedTasks = useMemo(() => {
    const result = [...localTasks].filter(task => {
      ...
      if (
        currentDiscipline !== 'all' &&
        task.discipline_id !== currentDiscipline
      )
        return false
      if (
        currentPriority !== 'all' &&
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-full overflow-hidden">
      <div className="flex flex-col gap-4">
        ...
        <div className="sm:hidden w-full">
          <Collapsible
            ...
            className="bg-secondary/30 rounded-xl border border-border/60 overflow-hidden shadow-sm transition-all"
          >
            <CollapsibleTrigger asChild>
              <Button
                variant="ghost"
                className="w-full flex items-center justify-between px-4 h-12 hover:bg-transparent"
              >
              ...
              <div className="pt-4 flex flex-col gap-3">
                ...
                {hasActiveFilters && (
                  <Button
                    ...
                    className="text-destructive h-9 gap-2 font-bold text-xs"
                  >
        ...
        <div className="hidden sm:flex flex-row items-center gap-4 w-full bg-secondary/20 p-2.5 rounded-xl border border-border shadow-sm">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Select
              ...
              onValueChange={v => updateFilter('status', v)}
            >
              <SelectTrigger className="h-10 w-30 bg-background border-border/60 text-xs font-semibold rounded-full hover:border-primary transition-colors">
                <SelectValue placeholder="Status" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Todos Status</SelectItem>
            ...
            <Select
              ...
              onValueChange={v => updateFilter('discipline', v)}
            >
              <SelectTrigger className="h-10 w-35 bg-background border-border/60 text-xs font-semibold rounded-full hover:border-primary transition-colors">
                <SelectValue placeholder="Disciplina" />
              </SelectTrigger>
              <SelectContent>
          ...
          {hasActiveFilters && (
            <Button
              ...
              size="icon"
              onClick={clearFilters}
              className="h-10 w-10 text-destructive shrink-0 hover:bg-destructive/10 rounded-full ml-1"
            >
        ...
        <div className="sm:hidden flex flex-col gap-2 p-3 bg-secondary/15 rounded-xl border border-border/40">
          <div className="flex gap-2">
            <Button
              ...
              size="sm"
              onClick={() => setIsBulkConfirmOpen(true)}
              className="flex-1 h-11 text-[10px] font-black uppercase gap-2 bg-primary text-white shadow-sm"
            >
            ...
            <Button
              ...
              size="sm"
              onClick={() => setIsReorderMode(!isReorderMode)}
              className="flex-1 h-11 text-[10px] font-black uppercase gap-2 border-2"
            >
              {isReorderMode ? (
                <Save className="h-4 w-4" />
              ) : (
      ...
      <DndContext
        ...
        onDragEnd={e => {
          ...
          if (over && active.id !== over.id) {
            ...
            if (
              processedTasks[oldIndex].is_pinned ===
                processedTasks[newIndex].is_pinned &&
              processedTasks[oldIndex].status ===
                processedTasks[newIndex].status
            ) {
            ...
            {processedTasks.length > 0 ? (
              processedTasks.map((task, index) => {
                ...
                const showMoveButton =
                  nextTask &&
                  nextTask.is_pinned === task.is_pinned &&
      ...
      <AlertDialog open={isBulkConfirmOpen} onOpenChange={setIsBulkConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Concluir Lista Visível?</AlertDialogTitle>
          </AlertDialogHeader>
          <p className="text-sm text-muted-foreground">
            Isso marcará todas as tarefas pendentes mostradas atualmente como
            concluídas.
          </p>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction
              onClick={async () => {
                const pendingIds = processedTasks
                  .filter(t => t.status === 'pending')
                  .map(t => t.id)
                await supabase
                  .from('tasks')
                  .update({ status: 'completed', is_pinned: false })
                  .in('id', pendingIds)
                setLocalTasks(prev =>
                  prev.map(t =>
                    pendingIds.includes(t.id)
                      ? { ...t, status: 'completed', is_pinned: false }
                      : t
                  )
                )
