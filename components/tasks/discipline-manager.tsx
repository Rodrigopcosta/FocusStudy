...
export function DisciplineManager({
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        'Isso excluirá a disciplina e poderá afetar tarefas vinculadas. Continuar?'
      )
    )
      return
    const supabase = createClient()
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <DialogHeader>
          <DialogTitle>
            {mode === 'create' ? 'Nova Disciplina' : 'Gerenciar Disciplinas'}
          </DialogTitle>
        </DialogHeader>
            ...
            <div className="border-t pt-4">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                {disciplines.map(d => (
                  <div
                    key={d.id}
                    className="flex items-center justify-between p-3 sm:p-2 rounded-lg bg-accent/50"
                  >
