...
export function PomodoroTimer({
  ...
  const syncTimer = useCallback(() => {
    ...
    if (savedIsRunning && savedEndTime) {
      ...
      setTimeLeft(
        savedIsBreak ? MODES[currentMode].break : MODES[currentMode].work
      )
    }
  }, [defaultMode, tasks])
  ...
  return (
    <div
      className={`${isFocusPage ? 'fixed inset-0 z-50 bg-background flex items-center justify-center p-4' : 'max-w-2xl mx-auto space-y-6 px-2'}`}
    >
      <Card
        className={
          isFocusPage
            ? 'w-full max-w-3xl border-none shadow-none bg-transparent'
            : ''
        }
      >
        <CardContent className="pt-6">
          {!isFocusPage && (
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-muted-foreground">
                {isBreak ? 'Hora do descanso!' : 'Foque nos estudos.'}
              </p>
            </div>
          )}
          ...
          <div className="relative flex items-center justify-center mb-8">
            <div
              className={`relative ${isFocusPage ? 'w-72 h-72 sm:w-96 sm:h-96' : 'w-56 h-56 sm:w-64 sm:h-64'}`}
            >
              <svg
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span
                  className={`${isFocusPage ? 'text-6xl sm:text-8xl' : 'text-4xl sm:text-5xl'} font-bold font-mono tracking-tighter tabular-nums`}
                >
                  {formatTime(timeLeft)}
                </span>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 mb-8">
            {isRunning ? (
              ...
              <Button size="lg" onClick={handleStart} className="min-w-35">
                ...
                {timeLeft === MODES[mode].work && !isBreak
                  ? 'Iniciar'
                  : 'Continuar'}
              </Button>
            )}
            ...
            <Button size="lg" variant="secondary" asChild className="min-w-35">
              <Link
                href={isFocusPage ? '/dashboard/pomodoro' : '/dashboard/focus'}
              >
                {isFocusPage ? (
                  <>
                    <Minimize2 className="mr-2 h-5 w-5" /> Sair do Foco
                  </>
                ) : (
                  <>
                    <Maximize2 className="mr-2 h-5 w-5" /> Modo Foco
                  </>
                )}
              </Link>
            </Button>
