    ...
    <div className="max-w-5xl mx-auto p-6 space-y-10 animate-in fade-in duration-500">
      <header className="flex justify-between items-center">
        ...
        <div className="bg-accent/20 p-3 rounded-2xl border text-right">
          ...
          <p className="text-xs font-bold text-primary italic">
            Novos: {dailyStats.newDone}/{MAX_DAILY_NEW} | Rev:{' '}
            {dailyStats.reviewsDone}/{MAX_DAILY_REVIEWS}
          </p>
        </div>
      </header>
      ...
      >
        <CardContent className="p-12 space-y-6">
          ...
          <Button className="bg-primary font-black rounded-2xl px-10 h-14 text-lg">
            {dailyStats.newDone >= MAX_DAILY_NEW ? 'CONCLUÍDO' : 'INICIAR'}{' '}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <PerformanceItem
          ...
          icon={<AlertCircle />}
          color="red"
          disabled={dailyStats.reviewsDone >= MAX_DAILY_REVIEWS}
          onClick={() => onStart('errei')}
        />
        <PerformanceItem
          title="Difícil"
          count={categories.dificil.count}
          icon={<Zap />}
          color="orange"
          disabled={dailyStats.reviewsDone >= MAX_DAILY_REVIEWS}
...
function PerformanceItem({
  ...
  return (
    <Card
      className={cn(
        'border-2 transition-all cursor-pointer bg-card rounded-3xl',
        styles[color],
